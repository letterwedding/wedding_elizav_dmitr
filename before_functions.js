    ...
    if (!payed || (payed && $('#domain_check').val() == '')) {
        $.post(ajax_url, {action: 'whois', domain: domains[iter]}, function (data) {
            ...
            if (d[0] != '1' && iter < (domains.length - 1)) {
                ...
                if ((typeof (main_domain) != 'undefined' && main_domain != 'wedwed.ru') || (typeof (main_country) != 'undefined' && main_country != '0' && main_country != '5') || ($('.ct-tpl_selector-item.active').attr('data-template_type') > 1 && $('.ct-tpl_selector-item.active').attr('data-template_type') != 5)) {
                    smdom = '.' + main_domain;
                }
